...
class TestMsvcrt < Test::Unit::TestCase

  include Shellwords

  def setup
    Shellwords.default_rule_set = Msvcrt
  end
  ...
  def test_cmdline_of_output_argv_exe
    skip unless OnWindows
    Examples.each do |cmdline, expected|
      assert_equal "#{OutputArgvExe} #{cmdline}", split_per_output_argv_exe(cmdline)[0]
    end
  end
  ...
  def test_against_shell32_dll
    skip unless OnWindows
    Examples.each do |cmdline, expected|
      assert_equal expected, split_per_shell32(cmdline)
    end
  end

  def split_per_shell32(cmdline)
    require_relative 'ffi_shell32_loader'

    Shell32.CommandLineToArgv cmdline
  end
end
